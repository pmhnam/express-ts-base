# Codebase Express with Typescript
...
- URL: Embed the version in the URL such as POST `/v1/stores`.
  Header
- Custom header:
  - Adding a custom `X-API-VERSION` header key by the client can be used by a service to route a request to the correct endpoint.
...
> Note: Use `?limit=-1` to get all records
...
> Note: Please use `encodeURIComponent()` to encode the value.

- `|` as `OR`
- `&` as `AND`
- `>=` as `GREATER THAN EQUALS`
- `<=` as `LESS THAN EQUALS`
- `>` as `GREATER THAN`
- `<` as `LESS THAN`
- `!=` as `NOT EQUALS`
